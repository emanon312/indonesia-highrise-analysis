# -*- coding: utf-8 -*-
"""同名近距合并优化：距离<25m 且 同名(非无名) → 合并（同一栋楼被登记成不同高度）。
在已有空间去重基础上补一轮，12城全做。保留信息最全+最高的代表点。
dry-run默认，--run写回。"""
import sys, os, math
...
def process(city, write):
    ...
    def score(r):
        ...
        if g and g!='—':s+=2
        return s
    pts=[(num(r[li]),num(r[oi]),num(r[hi]) or 0,str(r[ni]).strip() if r[ni] else '',score(r),idx) for idx,r in enumerate(rows)]
    # 贪心：按信息完整度+高度降序保留，删除<25m且同名的
    order=sorted(range(len(pts)),key=lambda i:(-pts[i][4],-pts[i][2]))
    ...
    for i in order:
        ...
        if p[0] is None or unnamed(p[3]):continue  # 无名点不作同名合并
            ...
            for dy in(-1,0,1):
                for j in grid.get((ck[0]+dx,ck[1]+dy),[]):
                    if not removed[j] and j!=i:
                        q=pts[j]
                        if not unnamed(q[3]) and q[3]==p[3] and dist(p,q)<R:
                            removed[j]=True
    keep_idx.sort()
    ...
    if write and deleted>0:
        import shutil
        bak=os.path.join(OUT,'..','backup_同名放宽前')
...
def main():
    ...
    print('='*50); print('  同名近距合并优化(同名+<25m) - '+('执行' if write else 'dry-run')); print('='*50)
    t0=t1=0
    for c in CITIES:
        b,a,d=process(c,write); t0+=b;t1+=a
        if d>0: print('  %-5s %d → %d (删%d)'%(c,b,a,d))
    print('-'*50)
